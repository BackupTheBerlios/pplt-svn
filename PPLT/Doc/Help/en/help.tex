...
\begin{document}
	...
	Or you can right-click on the device list and select the menu entry "Add Device".
	\par
	Now you'll see the device selection dialog. In this dialog all known devices are
	listed by there classes. You can open a class by double clicking the class.
	Now select the device you want to load and confirm it by double-click the device
	or click on the OK button. 
	\par
	Now you'll see the device setup dialog. There you can set all parameters needed to
	...
	parameter, keep a while with your mouse over the entryfeeld to get a
	tool-tip help. 
	\par
	If all going right, you'll see your device now in the device list. If an error 
	accours you can check out the logging window on the	bottom of the PPLT Center 
	window for the reasons.  
		
		
	\subsection{Unload a device}
	To unload a device simply right-click on the device in the devicelist. Now
	...
	logingwindow at the bottom of the application. 
	\par
	...
	create the new folder. If you want to create a folder on the root directory
	simple right click at a free space in the symboltree or at the 
	...
	if the folder shloud become editable. Rename the folder
	...
	You can move a folder into an other or at the root by drag 
	and drop.
	\par
	\textbf{Note}: You can't move a folder into a a sub folder
	...
	was empty the it shloud be removed now. 
	\par
	\textbf{Note}: The folder but be empty.
	
	
	\subsection{Create a symbol}
	To create a symbol you could right-click the folder where you want to
	create the symbol. Or, if you want to create the symbol at the root
	directory, right-click a free space inside the \textit{Symbol Tree}.
	Then select \textit{Add symbol} in the context menu. 
	\par
	Now you should see a dialog with a list of all loaded devices. Open the
	device you want the symbol be connected to by a double-click. Now you should
	see a list of all namspaces the device offers. Mostly there are only one
	...
	\par
	Now select the slot you want to use with a double-click. You can see a
	...
	address of name for the slot. This means that the application can't
	...
	the type of the symbol. Also you can set here the cashing time for
	...
	editable. Make your canges and confirme them by pressing RETURN.
	\par
	\textbf{Note}: You can't rename a symbol to a symbol already exists.
	In this case, the old name will be restored and a error message 
	will be shown in the logging window at the bottom of the
	application.
	...
	To get or set the value of a symbol, you can right-klick the symbol and
	...
	and then klick the \textit{Set} button. 
	...
	see a dialog where you can set all these things. Confirm you canges by clicking
	the OK button.
	...
	To create a user right-click the group where the new user should be member of
	and select \textit{Add user} in the context-meun.
	\par
	Now you should see a dialog where you can set the user-name and also
	his password. Fill up the form and confirm you settings by klicking the
	OK Button.
	\par
	\textbf{Note}: A user must be a member of a group! So you'll need
	to define a group where the you will be created in.
	...
	a dialog where you can type the new password. Confirm your changes by
	clicking the OK button.
	
	\subsecton{Delete a user}
	...
	If you want to create a subgroup right-klick on the group where you want
	to craete the subgroup. Else if you want to reate a (root) group right-click
	...
	To delete a group, right-klick the group and select \textit{Del group}
	in the context-menu. Now the gorup should be removed.
	\par
	\textbf{Note}: The group must be empty (no users of subgroup in it) to
	delete it.
	
	\subsection{Create a user-proxy}
	To create a proxy for an user, right-klick the group where you want to
	...
	Right-klick the user, who should become the new superuser
