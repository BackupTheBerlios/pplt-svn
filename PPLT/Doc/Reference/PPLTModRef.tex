...
In this chapter i will describe all avaiable PPLT modules. A PPLT module is
...
many (more flexible) core modules, is the easy handleing of a (single) simple
device insted of dealing with a couple of modules.

Technical, a PPLT module is an XML file describing how to combine the 
coremodules to get the support for a special device or system. 

For example; If you want to access the markers of a Siemens SIMATIC S7-200 
by the PPI bus you have to load the coremodule for the serial interface, the 
...
\PPLTMod{'PLC.S7-200'}. This module needs only 3 paremeters to setup the 
...
specific device, devided by a singel dot. For example: 
...
gererator, thats provide random value in different types. This is the 
simplest device. It needs no additional python libraries nor any special 
hardware to run. So it can easiely be used to test the PPLT.

\subsubsection{Parameters}
This device needs no parameters to be setted up. 

\subsubsection{Namespaces and slots}
This device provide only one namespace called \code{Generatior}. This 
...
first (\code{/S7}) contains two symbols of PLC markers and the secont folder
...
\code{/S7/SMB28} will be read. And at the end the number of recived bytes 
...
set the flowcontrol to \code{RTS/TSR} and set the speed to \code{57600} baud.
\end{notice}
...
Like devices servers are also grouped by classes. A full qualified servername
...
authentication so the \code{DefuatUser} attribute will be ignored.
...
This server exports the symboltree for the Java visualisation JVisu. 
(\url{http://jvisu.sourceforge.net}). The protocol used by \code{JVisuSocket} 
doesn't know any authentiaction so you need to set the \var{DefaultUser}
\textbf{carefully}.
...
This function closes a session opend by \function{logon}.
The attribute \var{SessionID} is the id returned by the
\function{logon}-functioncall.
\end{funcdescni}
...
to read.
The optional attribute \var{SessionID} specifies the session you may opend by 
