<div class="TextBody">
	...
	do the first steps. If you have any question related to this document
	feel free to send me an <a href="/en/Contact.html">e-mail</a>.</p>

	<p>I used a lot of PPLT specific terms in this text, so you may want to 
	...
	install first the <b>Python interpreter</b> version 2.3.0 or higher. You 
	can get it from <a href="http://www.python.org">python.org</a>.

	<p>The next is the GUI-library used by the PPLT tools like <i>PPLT Center</i>
	...
	<p>Some plugins also need extra Python-extentions. To find out what extentions are
	needed, please look at the documentaion of the single plugin. For the GSM, Simatic S7-200,
	Agilent and Panasonic modules you will need the <b>pyserial</b> extention from
	...
	use. For Windows you will have an executable installer so there should not be
	any problems.<br>
	Under Linux you should get the source packages, unpack and compile them. How 
	...
	If everything is alright you should see now something
	like this:
	...
	Under Linux you should simply type <b>PPLTC.py</b> at an open terminal. Because Python copy all scripts into 
	...
	<p>At the beginning this will be a little bit strange to handle. But later it is quite fast.</p>
	</div>


	<div class="Head">PPLT Center working circle</div>
	<div class="Text">
		A normal working circle will be to load at first the devices you want to access. Then you
		create the symbols and connect them to the devices. The last step should be to load 
		the servers.  

		<p>To load a device, right-click at the tab <i>Devices</i> or at an empty space
		...
		see the setup-dialog for the device you selected. Fill up the form and click 
		<i>OK</i>. If you need help for the parameters you have to set, keep with the
		mouse over the entry-field of the parameter an a tool-tip will be shown or
		take a look at the <a href="/en/Devices.html">device-reference</a>. 
		<b>Note:</b> you always have to set an alias. This alias will be used to
		identify the device later. If all parameters were right, the device 
		...
		please right-click at an empty space inside the symbol-tree or at the
		...
		what they are and what they do. You should need to care about the slots or
		slotranges. A simple slot has the single plug as icon the slot range the 
		double-plug. <br>
		...
		a wide range of values. If you select a slot almost everything is done. You 
		...
		<p>Starting a server is quite similar to loading a device. You right-click
		at an empty space in the server list or at the tab <i>Servers</i>. Then
		you select the server you want to start with a double click. Like 
		laoding a device you'll see now the setup-dialog for the server. Fill
		up the form and click <i>OK</i>. You'll always have to set an alias,
		...
		folder and all its subfolders. Leaving the slash means to 
		export the whole symboltree. Like in the setup-dialog of devices
		you'll get tool-tips aswell if you stay with your mouse over an entry. If all
		parameters were ok, the server will be started and shown at the 
		list.</p>
	<div>
