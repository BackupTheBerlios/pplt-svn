...
class MetaData:
	def __init__(self, Document, Lang, AltLang):
		...
		devnode = Document.documentElement;
		header  = Document.getElementsByTagName("Head")[0];
		
		self.__Version = Version.Version(str(devnode.getAttribute("version")));
		self.__Name    = str(devnode.getAttribute("name"));
		self.__ClassString = str(devnode.getAttribute("class"));
		...
		tmpNodeLst = getNodesByTagName(header.firstChild, "Description");
		for node in tmpNodeLst:
			self.__addModDescription(node);
		
		tmpNodeLst = getNodesByTagName(header.firstChild, "Require");
		for node in tmpNodeLst:
			self.__addRequire(node);
		
		tmpNodeLst = getNodesByTagName(header.firstChild, "Provide");
		for node in tmpNodeLst:
