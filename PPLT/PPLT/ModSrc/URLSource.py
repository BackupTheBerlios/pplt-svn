...
class URLSource:
    ...
    def __init__(self, DB, ReposURL, Proxy={}):
        ...
        self.__ModuleListURL = self.__BaseURL+"/"+".Modules";
        self.__DataBase = DB;
        self.__Links = list([]);
        
        #Open URL of modules-list:
        print self.__ModuleListURL;
        try: fd = urllib.urlopen(self.__ModuleListURL, proxies=self.__Proxies);
        except: 
            raise ImportException(_("Can't open URL %s. Maybe bad proxy or network not reachable.")%self.__ModuleListURL);
        
        # Parse module-list;
        try: Doc = xml.dom.minidom.parse(fd);
        except Exception, e: 
            fd.close()
            raise ParseException(_("Error while parse module-list at %s\n>>> %s")%(self.__ModuleListURL, str(e)));
        
        if not Doc.documentElement.localName == "ModuleListing":
            raise ParseException(_("Invalid Document."))
    ...
    def __ParseCoreMod(self, xmlNode):
        ...
        if xmlNode.nodeType == xmlNode.ELEMENT_NODE:
            if not xmlNode.hasAttribute("name"): 
                raise ParseException(_("Invalid format of module-list %s. (no name)")%self.__ModuleListURL);
            if not xmlNode.hasAttribute("class"): 
                raise ParseException(_("Invalid format of module-list %s. (no class)")%self.__ModuleListURL);
            if not xmlNode.hasAttribute("version"): 
                raise ParseException(_("Invalid format of module-list %s. (no version)")%self.__ModuleListURL);
            if not xmlNode.hasAttribute("file"): 
                raise ParseException(_("Invalid format of module-list %s. (no file-name)")%self.__ModuleListURL);
    ...
    def __ParsePPLTMod(self, xmlNode):
        ...
        if xmlNode.nodeType == xmlNode.ELEMENT_NODE:
            if not Type in ("Server", "Device"):
                raise ParseException(_("Invalid element (%s) in list.")%Type);
            if not xmlNode.hasAttribute("name"): 
                raise ParseException(_("Invalid format of module-list %s. (no name)")%self.__ModuleListURL);
            if not xmlNode.hasAttribute("class"): 
                raise ParseException(_("Invalid format of module-list %s. (no class)")%self.__ModuleListURL);
